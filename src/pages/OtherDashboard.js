          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-xs text-slate-400">Other Dashboard</span>
              </div>
            </div>
            <button 
          ...
          <div className="mb-4">
            ...
            <ul className="space-y-1">
              ...
              <li
                ...
                onClick={() => { handleCategorySelect("MTN"); setIsOpen(false); }}
              >
                <img src="https://images.seeklogo.com/logo-png/9/1/mtn-logo-png_seeklogo-95716.png" className="w-5 h-5 rounded" alt="MTN" />
              ...
              <li
                ...
                onClick={() => { handleCategorySelect("TELECEL"); setIsOpen(false); }}
              >
                <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTl4R7lA1tlSlrBzf9OrDXIswYytfI7TfvC0w&s" className="w-5 h-5 rounded" alt="Telecel" />
              ...
              <li
                ...
                onClick={() => { handleCategorySelect("AIRTEL TIGO"); setIsOpen(false); }}
              >
                <img src="https://play-lh.googleusercontent.com/yZFOhTvnlb2Ply82l8bXusA3OAhYopla9750NcqsjqcUNAd4acuohCTAlqHR9_bKrqE" className="w-5 h-5 rounded" alt="AirtelTigo" />
        ...
        <header className="bg-white/80 backdrop-blur-lg border-b border-slate-200 sticky top-0 z-30">
          ...
          <div className="md:hidden p-4">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                <AgentNotifications />
                <button className="relative p-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 text-white shadow-lg hover:shadow-xl transition-all" onClick={() => setIsCartOpen(true)}>
          ...
          <div className="hidden md:flex lg:hidden p-4 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="relative p-2.5 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 text-white shadow-md hover:shadow-lg transition-all" onClick={() => setIsCartOpen(true)}>
                <ShoppingCart className="w-5 h-5" />
          ...
          <div className="hidden lg:flex p-4 items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="relative p-3 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 text-white shadow-lg hover:shadow-xl hover:scale-105 transition-all" onClick={() => setIsCartOpen(true)}>
        ...
        {userRole === "Other" ? (
          loading ? (
            ...
            <main className="flex-1 overflow-y-auto bg-gradient-to-br from-slate-50 to-slate-100 p-4 md:p-6 lg:p-8">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  <div>
                    <h2 className="text-2xl md:text-3xl font-bold text-slate-900">
                      {selectedCategory ? `${selectedCategory} Products` : "Other Products"}
                    </h2>
                    <p className="text-slate-500 mt-1">
                      {products?.filter(p => p.name.includes("OTHER")).length || 0} products available
                    </p>
                  </div>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all ${
                          (cat === "All" && selectedCategory === null) || selectedCategory === cat
                            ? cat === "MTN" ? "bg-yellow-500 text-white" : cat === "TELECEL" ? "bg-red-500 text-white" : cat === "AIRTEL TIGO" ? "bg-blue-500 text-white" : "bg-purple-500 text-white"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
                {products
                  ?.filter((product) => {
                    if (selectedCategory === null) return product.name.includes("OTHER");
                    return product.name.includes(selectedCategory) && product.name.includes("OTHER");
                  })
                  .sort((a, b) => {
                    const getSize = (p) => {
                      const match = (p.name + " " + p.description).match(/(\d+)\s*GB?/i);
              ...
              {/* Empty State */}
              {products?.filter(p => p.name.includes("OTHER")).length === 0 && (
          ...
          <div className="flex-1 flex justify-center items-center bg-gradient-to-br from-slate-50 to-slate-100">
            <div className="text-center p-8">
              ...
              <p className="text-slate-500">Only Other Dashboard users can view these products.</p>
            </div>
          </div>
        )}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-purple-200">{cart.length} item{cart.length !== 1 ? 's' : ''}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {cart.map((item, index) => (
                    <motion.div key={index} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-colors">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${item.product.name.includes("MTN") ? "bg-yellow-100" : item.product.name.includes("TELECEL") ? "bg-red-100" : item.product.name.includes("AIRTEL") ? "bg-blue-100" : "bg-purple-100"}`}>
                        <span className="text-lg font-bold">{item.product.name.includes("MTN") ? "M" : item.product.name.includes("TELECEL") ? "T" : item.product.name.includes("AIRTEL") ? "A" : "O"}</span>
                      </div>
